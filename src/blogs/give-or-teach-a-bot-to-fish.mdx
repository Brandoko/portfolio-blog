...
The phrases "Give a Bot a Fish" and "Teach a Bot to Fish" come from an excellent [prompt engineering guide](https://github.com/brexhq/prompt-engineering#give-a-bot-a-fish) from Brex.

These phrases are ways of describing how Large Language Models (LLMs) can understand and interact with the outside world.

The source code from the examples in this post can be found [here](https://github.com/Brandoko/gpt-fish)
...
LangChain has done the grunt work of setting up the prompts and interfaces for providing a LLM a set of tools and getting it to enter an action-observation loop by using the tools to accomplish a goal. Let's try it out.
