...
Clear out the pre-made body styles in `src/app/globals.css`. Your `globals.css` file should contain the only the following:
...
Visit localhost:3000/blogs/hello to see the results.
...
Your mini blog post should be looking better now.

## Adding blog posts

Create a `first-blog.mdx` inside the `src/blogs` folder.

Install the `gray-matter` package to parse the frontmatter values from the MDX files.

Create a `get-blogs.ts` file in a utils folder that will contain a function reading all the blogs.
...
Time to see the result of our work! Go to localhost:3000/blogs/first-blog and you should see your first-blog.mdx.
...
Finally add this following export to `src/app/blogs/[slug]/page.tsx` to tell Next that any page that was not statically built should return a 404 instead of trying to dynamically render:
